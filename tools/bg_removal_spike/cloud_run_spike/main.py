"""배경 제거 재개 — 배포 콜드스타트 실측 스파이크
(docs/task_background_removal_v1.md §2-3 다음 단계).

**이 함수는 트리거 배선이 아니다.** wardrobe_images/ 업로드를 감지하지
않고, Firestore/Storage 어느 것도 읽거나 쓰지 않는다. 목적은 딱
하나 — "실제로 배포된 Cloud Run(Firebase Functions 2nd gen) 컨테이너,
최소 인스턴스 0, u2netp 번들링" 조건에서 콜드스타트가 로컬 실측
하한값(1.29초)에 컨테이너 부팅 시간을 더하면 얼마가 되는지 재는 것.
측정 후 삭제 예정(firebase functions:delete).

모델 번들링: u2netp.onnx를 이 디렉터리에 직접 두고(빌드팩이 소스
디렉터리 전체를 컨테이너에 포함) U2NET_HOME을 이 디렉터리로
지정해 rembg가 매 요청/콜드스타트마다 네트워크로 받지 않고
로컬 파일을 바로 쓰게 한다.

인증: invoker를 이 프로젝트의 Admin SDK 서비스 계정으로만 제한한다
(공개 엔드포인트 아님) — 호출 시 그 서비스 계정의 ID 토큰이 필요하다.

입력: POST 바디에 원본 이미지 바이트(로컬 스파이크와 같은 표본 —
실데이터 아님). 바디가 없으면 콜드스타트 시간만 재기 위한 내장
1x1 JPEG로 대체한다.
...
os.environ.setdefault("U2NET_HOME", BASE_DIR)
...
# 로드된다 — 요청마다 다시 로드하지 않는다(실제 트리거 구현과 같은
...
def bg_removal_coldstart_spike(req: https_fn.Request) -> https_fn.Response:
    ...
    result = remove(buf.getvalue(), session=_session)
