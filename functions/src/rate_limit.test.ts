...
const CONFIG = {textLimit: 3, imageLimit: 2};
...
run("한도 미만 — 계속 허용하며 카운트만 누적", () => {
  const now = new Date("2026-08-06T10:15:00Z");
  const state: RateLimitState = {bucket: utcHourBucket(now), textCount: 1, imageCount: 0, rejectedCount: 0};
...
run("한도 직전(count == limit-1) — 허용하고 카운트가 한도에 도달", () => {
  const now = new Date("2026-08-06T10:15:00Z");
  const state: RateLimitState = {bucket: utcHourBucket(now), textCount: 2, imageCount: 0, rejectedCount: 0};
  const decision = evaluateRateLimit(state, now, "text", CONFIG);
  assert.strictEqual(decision.allowed, true);
...
run("한도 도달(count == limit) — 거부하고 rejectedCount만 증가, textCount는 그대로", () => {
  const now = new Date("2026-08-06T10:15:00Z");
  const state: RateLimitState = {bucket: utcHourBucket(now), textCount: 3, imageCount: 0, rejectedCount: 0};
  const decision = evaluateRateLimit(state, now, "text", CONFIG);
...
run("한도 초과 직후 연속 거부 — rejectedCount가 계속 누적", () => {
  const now = new Date("2026-08-06T10:15:00Z");
  const state: RateLimitState = {bucket: utcHourBucket(now), textCount: 3, imageCount: 0, rejectedCount: 4};
  const decision = evaluateRateLimit(state, now, "text", CONFIG);
  assert.strictEqual(decision.allowed, false);
  assert.strictEqual(decision.nextState.rejectedCount, 5);
});

run("텍스트/이미지 카운트는 독립 — 이미지가 한도여도 텍스트는 영향 없음", () => {
  const now = new Date("2026-08-06T10:15:00Z");
  const state: RateLimitState = {bucket: utcHourBucket(now), textCount: 0, imageCount: 2, rejectedCount: 0};
  const decision = evaluateRateLimit(state, now, "text", CONFIG);
  assert.strictEqual(decision.allowed, true);
  assert.strictEqual(decision.nextState.textCount, 1);
...
run("버킷 전환(같은 날 다음 시간) — 카운트·거부수 전부 리셋 후 1로 시작", () => {
  ...
  const state: RateLimitState = {bucket: utcHourBucket(prevHour), textCount: 3, imageCount: 2, rejectedCount: 9};
...
run("버킷 전환(자정·월·연도 경계) — 문자열 버킷 키가 올바르게 갈린다", () => {
  ...
  const state: RateLimitState = {bucket: utcHourBucket(dec31), textCount: 3, imageCount: 0, rejectedCount: 1};
...
run("같은 버킷 안에서는 리셋되지 않는다(경계 오탐 방지)", () => {
  ...
  const state: RateLimitState = {bucket: utcHourBucket(t1), textCount: 3, imageCount: 0, rejectedCount: 0};
