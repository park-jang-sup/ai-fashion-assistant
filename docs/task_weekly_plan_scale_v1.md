...
- **52.8%(15,848자) 수치의 출처**: `docs/task_hardening_v2.md`
  ...
  없어 미상). 오늘 계산치([총합] 14,328, 피드백 제외)가 그 실측(15,848)과
...
- `payload_limit.ts` 위반은 `details` 스키마가 다르다
  ...
  실측 [총합] 14,328자 ≈ 14KB) 지금은 실질적 공백이 아니다. 다만 두
...
→ **약 212벌**(현재 140벌 대비 +72벌), 문자수로는 [총합, 피드백
제외 가정] **약 21,000자**.

**방식 2 — 실측 최대 벌크 등록(66벌, §2(c))을 흡수할 여유로
역산.** 30,000자 도달 예상 303벌(§2(c))에서, 이 계정이 실제로
겪은 **가장 큰 단일 등록 이벤트(2026-07-12, 66벌)**를 여유로
뺀다: `303 − 66 = 237벌` → 문자수 [총합, 피드백 제외 가정]
`856 + 237 × 96.24 ≈ 23,665자 ≈ 78.9%`.

**두 값을 비교해 더 보수적인(먼저 울리는) 쪽을 채택한다**: 방식
1(70%, 약 212벌)이 방식 2(78.9%, 약 237벌)보다 낮다 — 즉 방식
2로만 정했다면 이미 등록된 70% 트리거보다 늦게 울렸을 것이다.
**따라서 기존에 등록된 70%(≈212벌, [총합] ≈21,000자)를 그대로
경고 임계값으로 채택한다** — 오늘의 실측이 그 값을 무효화하지 않고
오히려 같은 자릿수(70~79%대)로 재확인해 줬다는 뜻으로 기록한다.
방식 2(237벌, 78.9%)는 폐기하지 않고 "이 값을 넘으면 과거 최대
벌크 등록 규모조차 흡수할 여유가 없다"는 2차 참고선으로 함께
등록한다.

**[확인, 2026-08-14] 임계값의 기준이 실제로 검사되는 값과 일치하는지 코드로 대조.**
`request_shape.ts`의 `evaluateRequestShape`는 `part["text"].length`를
합산하고(151행), 그 합계를 상한과 비교한다(165행) — 주간 플랜
요청은 텍스트 파트가 하나뿐이라
(`gemini_service.dart:742-745`), 이 합계는 **카탈로그+스케줄+
피드백 섹션+템플릿 전부를 포함한 진짜 총합**이다. 위 212벌·
21,000자는 이 진짜 총합이 아니라 **피드백을 0으로 가정한
근사 총합**(856 고정 오버헤드에 피드백이 안 들어감, §2(a))
위에서 계산됐다 — 실제 요청에 피드백이 섞이면(관측된 사례는
2026-08-11 실측에서 308자, `task_hardening_v2.md`) 같은 30,000자
예산을 피드백이 먼저 갉아먹으므로, **진짜 트리거 시점은 212벌보다
근소하게(피드백 308자 ÷ 벌당 96.24자 ≈ 3벌) 더 일찍 올 수 있다.**
**임의로 임계값을 고치지 않는다 — 이 가정(피드백 제외)을 명시적으로
등록만 한다.** 오차가 현재 여유(70벌 이상) 대비 작아 임계값
자체를 무효화하지는 않는다고 판단하나, 재검토 시점이 임계값에
가까워지면 이 근사를 재확인해야 한다.
...
**`catalog` 문자수 [카탈로그만](13,472)와 정확히 일치**한다 —
코드가 실제로 기록하는 값이 `catalog.length`(카탈로그만, 스케줄·
템플릿 오버헤드 제외)라는 설계 그대로 동작함을 실기기로 확인했다.
**배선(Firestore까지 닿는가)과 계산(그 값이 맞는가)이 이
일치 하나로 함께 검증됐다.** (직전 `weekly_planned` 문서,
2026-08-10 03:52:06 UTC에는 이 필드가 없음 — 이번 수정 이전
문서라 정상.)

**주의(다음에 참고할 것)**: 이 필드는 `catalog.length`만 [카탈로그만]
이지 `request_shape.ts`가 실제로 세는 `totalTextChars`(스케줄·
피드백·템플릿 전부 포함)와는 다르다 — 오늘 §2(a) 계산의 "합계
[총합, 피드백 제외] 14,328자"와 혼동하지 않는다. 전체 텍스트 길이를
...
> `weeklyPlanCatalogChars` 추세(이 필드는 [카탈로그만] 값이다)도
> 함께 보고, §0(c)의 경고 임계값([총합] 약 212벌·21,000자, 오늘
> 140벌·[카탈로그만] 13,472자 대비)에 얼마나 가까워졌는지 대조할
> 것 — `weeklyPlanCatalogChars`(카탈로그만)에 고정 오버헤드
> 856자를 더해야 임계값과 같은 기준([총합])이 된다.
