...
이 지시서의 4절 판단 기준은 실측에서 빗나갔다. 무채색 보너스 게이팅은
이 옷장에서 효과가 0이었고(9개 태그 전부 current=proposed), isFallback이
안 켜지던 진짜 원인은 두 가지 다른 것이었다.

- skeleton 슬롯이 3개라 아우터·신발 중 하나가 항상 탈락 (3→4로 수정)
- _lowScoreFloor=60이 관측 최저점 65보다 낮아 도달 불가 (70으로 수정)

측정 하네스가 잘못된 수정을 막고 진짜 원인을 드러낸 사례.
