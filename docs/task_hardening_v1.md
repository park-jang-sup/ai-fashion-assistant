...
**[정정 2026-08-08]** 3번의 "옷 삭제"가 검증하던 함정(위 §86행
"delete 요청에서 `request.resource`가 null이라 `allow write`가
막아버릴 수 있다")은 `deleteWardrobeImage()`가 실제로 호출돼야만
드러난다. 2026-08-08 세션에서 `wardrobe_screen.dart`의
`_confirmDelete`가 `demo_wardrobe`·다른 사용자 사본과 Storage
파일을 공유하는 구조 때문에 오삭제 위험이 있음이 확인돼, 이
함수 호출 자체를 삭제 흐름에서 뺐다(`docs/handoff_2026-08-07.md`
§4 "삭제 시 Storage 고아" 참고). 그 결과 **지금은 "옷 삭제"를
실기기에서 확인해도 이 함정이 더 이상 검증되지 않는다** — Storage
쪽으로 삭제 요청 자체를 안 보내기 때문이다. `storage.rules`의
관련 규칙은 안 바뀌었지만(코드는 그대로 있음, "함수 자체는 지우지
않는다"는 원칙에 따름) 지금 이 규칙을 실제로 두드리는 호출부가
없다는 뜻 — 근본 해결(시드 시 파일 복사 + 경로 분리)로
`deleteWardrobeImage()`를 다시 쓰게 되면 이 검증 항목도 다시
의미를 갖는다.
...
**[정정 2026-08-08]** 이 지적은 해소됐다 — 지금 `wardrobe_screen.dart`의
`_seedDemoWardrobe`는 숫자 없는 "데모 옷장을 준비하고 있어요, 잠시만
기다려 주세요"를 쓴다(코드 확인, 2026-08-08). 다만 이 문서가 당시
인용한 "실제 데모 옷장은 119벌"도 지금은 현재값이 아니다 — 그 사이
112벌(2026-08-04~05경 축소)을 거쳐 오늘 재시드로 118벌이 됐다(§4/§6의
`handoff_2026-08-07.md` 참고). 즉 이 절이 지적한 숫자(118 하드코딩,
119 실측) 둘 다 이제 과거 스냅샷이고, 문제였던 "숫자를 하드코딩한다"는
패턴 자체가 사라졌으니 더 이상 갱신할 숫자가 없다.
...
**[정정 2026-08-10, task_hardening_v2 트랙에서] 2번 항목의 구분 기준을
다시 세운다.** 이 구분은 당시 빌드·설치 도구 접근 가능 여부가 확인되지
않은 상태에서 세워졌다. 이후 `adb`가 표준 Android SDK 경로에 있고
기기가 연결되어 있음이 실제로 확인되어 전제가 바뀌었다.

- **"릴리스 빌드 + `install -r`"** → 요청 시 수행 가능. 다만 매번
  사용자 승인을 받는다(수행 직전 확인 프롬프트를 띄운다) — 임의로
  판단해 수행하지 않는다.
- **"`uninstall` 후 `install`"** → **여전히 수행하지 않는다.** 익명
  uid와 WorkManager 예약이 리셋되어 관측 표본이 끊기며, 5.11.4절이
  기록한 식별자 분화 사고의 조건이 그대로 재현된다.
- **"앱 조작(화면 탭, 기능 실행)"** → 여전히 사용자만 할 수 있다.
- **"콘솔 확인"** → 여전히 사용자만 할 수 있다.

즉 이 구분의 기준을 **"도구 접근 가능 여부"가 아니라 "되돌릴 수
있는가"**로 다시 세운다. `install -r`은 되돌릴 수 있고(다시 설치하면
그만), `uninstall`은 되돌릴 수 없다(리셋된 식별자와 예약은 복구되지
않는다). 이는 `task_hardening_v2.md` §1이 트랙 전체의 순서 기준으로
쓴 "되돌리기 비용"과 같은 기준이며, 그 기준이 작업 분담에도 적용된다는
것을 여기 명시한다.

이 갱신의 계기: 사용자가 릴리스 빌드+설치를 요청했을 때, 확인 프롬프트
과정에서 `install -r`과 `uninstall`의 차이를 스스로 구분해 제안한 것이
계기였다 — 이 규약이 원래 금지한 것("릴리스 빌드+설치" 전체)과 실제로
위험한 것(`uninstall`만)이 달랐고, 그 차이를 실행 직전에 발견했다.
