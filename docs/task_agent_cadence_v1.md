...
**[정정 2026-08-15, 이어서] 위 "미완, 등록만" 문단은 그 시점(§8
설계 직후) 기준으로는 맞았으나 이후 갱신되지 않아 낡았다 —
§8-1에서 착수 전 확인 결과 서버 게이트 없이는 이 트랙의 목적이
성립하지 않음을 확인하고, 서버 측 게이트(`evaluateCadenceGate`,
`functions/src/cadence_gate.ts`)를 같은 세션에서 추가 구현했다
(커밋 `ef76b4b`). §9-4에서 배포·실기기 검증까지 완료했다 — 연속
호출로 게이트가 실제로 발화를 막는 것을 Cloud Functions 로그
(`cadenceGate로 보류`)로 확인했다. **이 트랙은 (a)(b) 전부 종료다.**
아래 두 문단(원문)은 착수 시점의 기록으로 보존한다 — 서두의 요약이
본문(§8-1·§9-4)의 갱신을 따라가지 못했던 사례 자체를, 논문
6.5.7절(문서 3층 비동기화) 규칙(14)의 실사례로 등록해 둔다.
...
### 7-2. [갱신 2026-08-15] 시연 절차 — §9-4 실기기 검증으로 확정

§9-4에서 실제로 실행하고 확인한 절차를 그대로 옮긴다. **몇 년 뒤
이 문서만 보고 재현 가능해야 한다는 기준으로 적는다.**

**전제 조건(시연 시작 전에 미리 확인)**:
- 대상 계정의 "최근 5건"이 무반응 3건 이상인지 §2-1 방식대로
  재확인한다(시간이 지나며 창이 밀릴 수 있다, §7-1이 실제로 겪은
  일). 이 문서 작성 시점 기준 `JmllppO9`·`yPyw4DC2`가 충족,
  `BDDOIl08`은 미충족.
- 기기가 그 계정으로 로그인돼 있어야 한다. 아니면 로그아웃 후
  전환한다.

**절차**:

1. **(선택) 서버 게이트 효과를 보이려면** 캘린더에 향후 3일 이내
   일정 1건을 등록해 둔다(TPO 태그 아무거나). 이미 `planned` 일정이
   있으면 생략.
2. 설정 화면 → **"즉시 실행(테스트)"** 버튼을 누른다(WorkManager
   one-off, `force:true` — 기존 10~12시간 대기 없이 즉시 판정을
   태운다).
3. 몇 초 뒤 **"AI 비서 활동 내역"** 화면을 연다. "오늘" 섹션
   최상단에 두 줄이 보인다:
   - `발화 간격을 N시간에서 M시간으로 조정합니다`
   - `최근 추천 5건 중 K건이 반응 없음을 감지했습니다`
   (표시 순서는 최신순이라 "조정합니다"가 위, "감지했습니다"가
   아래다 — 기록 순서(감지 먼저)와 반대이니 헷갈리지 말 것,
   §9-4 4번 참고.) **이 두 줄이 이번 트랙의 핵심 산출물이다.**
4. **(선택, 서버 게이트까지 보이려면)** 설정 화면에 서버 발화를
   즉시 유발하는 버튼이 없으므로, 심사 환경에서는 3시간 자연
   주기를 기다리거나 이 문서 §9-4의 REST 호출 절차(커스텀 토큰 →
   ID 토큰 교환 → `triggerScheduledCheckTest` 직접 호출, 개발자
   전용)로 대체한다 — 사용자에게 노출되는 앱 내 버튼이 아니므로
   심사 시연에는 3번까지만 보여주는 것을 기본 시나리오로 삼는다.

**무엇이 보이는가 — 요약**: 3번 화면 캡처 한 장으로 "시스템이
자기 행동의 결과(추천 5건 중 K건 무반응)를 관찰하고, 그 판단
근거와 결과를 스스로 서사로 남긴다"는 것이 증명된다. 서버 게이트
(간격이 실제로 발화 빈도를 줄인다는 것)는 화면으로 즉시 보여줄
수단이 없다 — §9-4가 REST 호출로 대신 검증했고, 그 결과(로그의
`cadenceGate로 보류`)를 캡처해 보조 자료로 쓸 수 있다.
...
### 9-4. (b) 배포·실기기 검증 (2026-08-15)

**배포 순서**: 서버(`ef76b4b`, §8-1 서버 게이트 포함) 배포 →
`flutter build apk --release` → `adb install -r`.

- **서버 배포 로그 — 통과.** delete 0건, update 9건
  (`callGeminiText`/`sendTestPush`/`beginFittingAttempt`/
  `generateFittingImage`/`getSignedImageUrls`/`revokeTokenOnUpload`/
  `triggerScheduledCheckTest`/`scheduledProactiveCheck`/
  `sweepStorageTokens`), `bg_removal_on_upload` skip — hardening §2
  절차와 동일 패턴.
- **클라이언트 재빌드·설치·`lastUpdateTime` 대조 — 통과.** 대상 커밋
  `ef76b4b`. 설치 완료 `10:31:57Z`, 기기 `lastUpdateTime=2026-08-15
  19:31:56`(KST → `10:31:56Z`) — 1초 이내 일치. 기기 `R3CW10DF8CW`.
  (중간에 adb 데몬이 연결을 놓쳐 기기 재연결 1회 발생 — `adb
  kill-server`/`start-server`로 해소, 검증 자체에는 영향 없음.)

**시연 계정 전환 — `yPyw4DC2`로 로그인.** §7-1 정정대로 `BDDOIl08`은
아직 무반응 1건뿐이라 부적합(72시간 문턱을 아직 못 채움, §2-1).
`yPyw4DC2`는 재확인 시점(검증 직전)에도 무반응 5/5 유지 —
사용자가 이 기기에서 `yPyw4DC2`로 로그아웃/로그인.

**확인 결과**:

1. **백그라운드 에이전트 실행 시 판정이 도는가 — 통과.** 설정
   화면의 "즉시 실행(테스트)" 버튼(WorkManager one-off,
   `force:true`) 탭 → `agent_meta.invokeCount` 38→39,
   `lastRunAt` 갱신(`10:34:27Z`) — 실행 자체는 확인.
2. **`agent_meta`에 조정값이 기록되는가 — 통과.**
   `adjustedIntervalHours=12`(기본 10시간에서 무반응 5건으로 2배,
   상한 클램프 없이 정확히 20이 아니라 12로 계산된 것도 아니고
   —10×2=20을 12로 클램프한 결과), `lastCadenceReason`="최근 추천
   5건 중 5건이 반응 없음을 감지했습니다", `lastCadenceCheckAt`
   동일 시각.
3. **활동 내역 화면에 판단 로그가 뜨는가 — 통과, 이번 작업의
   핵심 목적.** Firestore `agent_logs`에 `cadence_signal_detected`
   ("최근 추천 5건 중 5건이 반응 없음을 감지했습니다")→
   `cadence_adjusted`("발화 간격을 10시간에서 12시간으로
   조정합니다") 순으로 18ms 간격 기록. **실제 "AI 비서 활동 내역"
   화면에서 스크린샷으로 확인** — "오늘" 섹션에 두 메시지가
   그대로 렌더링됨(화면은 최신순 정렬이라 "조정합니다"가 위,
   "감지했습니다"가 아래로 뜬다 — 기록 순서와 화면 표시 순서가
   반대라는 점을 §7 시연 절차에 메모해 둔다). 감지와 조정이
   분리되어 각각 별도 말풍선으로 보인다 — 설계 의도(§6-1)대로다.
   원본 스크린샷: `docs/evidence_cadence_activity_log_2026-08-15.png`.
4. **조정 후 실제 발화 간격이 바뀌는가 — 서버 쪽은 실기기(REST
   호출)로 통과, 클라이언트 쪽은 단위 테스트로 확정(실기기는
   12시간을 기다려야 해 이번엔 관측하지 않음, 아래 참고).**
   - **서버**: 사용자가 `yPyw4DC2`에 2026-08-17 결혼식 일정을
     추가한 뒤, `triggerScheduledCheckTest`를 연속 2회 호출.
     1차(`10:45:40Z`) `{"triggered":true}` — 발송 성공,
     `serverLastSentAt` 신규 기록. 2차(`10:45:49Z`, 9초 후)
     `{"triggered":false}` — Cloud Functions 로그로 원인을
     확정: `"cadenceGate로 보류(조정된 간격 미달)"`. **같은
     대상 날짜(2026-08-17)가 여전히 미확인 상태인데도(추천이
     안 생겼으므로 `findNextUntriggeredDate`는 여전히 그 날짜를
     찾아냈을 것) 게이트가 별도로 막았다는 뜻** — 대상 날짜
     유무와 간격 게이트가 서로 다른 이유로 각각 작동함을 로그로
     구분해 확인했다.
   - **클라이언트**: 12시간을 실제로 기다려 관측하는 대신
     `test/background_agent_test.dart`에 `minInterval:
     Duration(hours: 12)`를 직접 넘기는 경계 테스트 2건을
     추가해 확정했다(3시간 경과=미실행, 13시간 경과=실행,
     `e408201`) — `effectiveMinInterval`이 `adjustedIntervalHours`를
     정확히 읽어 넘긴다는 것은 §8-1에서 코드로 이미 확인했으므로,
     남은 것은 `shouldRunNow` 자체가 그 값을 올바르게 쓰는지뿐이고
     이는 순수 함수라 실기기 대기 없이 결정적으로 검증된다.

**결론 — §9-3(a)에 이어 §9-4(b)까지 이번 작업의 목적("자기 행동의
결과를 보고 발화 정책을 스스로 바꾸며, 그 판단이 관찰 가능하게
드러난다")이 코드·Firestore·화면 세 층위 모두에서 실측으로
확인됐다.**
