...
## 10. 다음 트랙 후보 (2026-08-13, 인계 기록만 — 착수 금지)

**1. [최우선] 폴백 응답 후보의 최종 추천 구조적 우위 해소.** §9 (b)·
`handoff_2026-08-07.md` §4 "폴백 응답 후보가 최종 추천 경쟁에서
구조적으로 유리하다" 행 참고. 이번 트랙이 반쪽으로 남긴 것 —
`bestMatch` 갱신에 `judgeCandidate`의 신뢰 판정을 반영하는 2단
비교(신뢰 후보 우선, 없을 때만 판정 불가 후보를 임시 채택)를
설계·구현. 재시도 태스크 생성 경로(`outcome==null`)와 맞물려야
한다.

**2. 로드맵 5번 — 신규 등록 아이템의 실시간 임베딩.** 사용자
판단으로, 자기평가 축은 여기서 접고 이 축으로 넘어간다 — 최종
목표(홈쇼핑·소규모 매장 옷과 내 옷장을 연결)의 전제 조건이기
때문이다. **아래는 인계 기록이다 — 조사·구현에 착수하지 않는다.**

### (a) 현재 벡터 부여 경로

수동 3단계 배치이고, 신규 등록과 무관하게 완전히 분리돼 있다
(코드 확인, 2026-08-13):
1. `tools/export_for_kaggle/export.py` — 개발자가 로컬에서 실행,
   Admin SDK로 옷장 이미지·메타데이터를 zip으로 내보낸다.
2. 그 zip을 Kaggle 노트북(`kaggle_embedding_comparison.ipynb`)에
   올려 FashionCLIP(`patrickjohncyh/fashion-clip`, 512차원)을
   Kaggle GPU에서 돌려 `embeddings.json`을 만든다.
3. `tools/backfill_embeddings/backfill.py --apply` — 개발자가
   로컬에서 실행, `embeddings.json`을 Firestore `wardrobe.{id}.embedding`
   필드로 백필한다(`--force` 없으면 이미 있는 문서는 스킵).

**세 단계 전부 개발자만 실행 가능하고, 새 옷 등록 시점에 자동으로
걸리는 트리거가 없다.** 그래서 이 배치를 마지막으로 돌린 시점
이후 등록된 모든 아이템(사용자가 직접 등록한 옷 전부 포함)은
**구조적으로 영원히 `embedding` 필드가 없다** — 다음에 누군가
같은 3단계를 다시 돌리기 전까지는.

### (b) 왜 이것이 최종 목표의 전제인가

최종 목표(홈쇼핑·소규모 매장의 옷을 사용자 옷장과 연결)를 이루려면
상점 옷의 벡터와 내 옷의 벡터를 같은 임베딩 공간에서 비교(코사인
유사도 등)해야 한다. 상점 옷 벡터를 아무리 잘 만들어도, **비교
대상인 내 옷 벡터가 없으면 비교 자체가 성립하지 않는다.** 지금은
배치를 돌린 시점의 스냅샷만 벡터를 갖고, 그 이후 등록된 옷(가장
최근에 산 옷, 사용자가 지금 당장 매칭해 보고 싶어할 옷)은 구조적으로
빠진다 — "비슷한 옷" 기능이 사용자가 최근에 관심 가질 만한 아이템일수록
못 다루는 역설이 생긴다.

### (c) 미해결 핵심 질문 — FashionCLIP을 어디서 실행할 것인가

**배경 제거(`functions_bg_removal`)가 겪은 콜드스타트·상시 인스턴스
비용 문제와 같은 축이다.** FashionCLIP은 Kaggle GPU에서 돌리던
모델이다 — 이걸 실시간(등록 즉시) 파이프라인에 넣으려면 CPU
전용 서버 환경(Cloud Functions/Run)에서 추론 시간이 얼마나
걸리는지, GPU 없이도 실용적인지부터 확인해야 한다. 배경 제거가
이미 겪은 것과 같은 트레이드오프(콜드스타트 지연 vs 상시 인스턴스
비용)가 그대로 재현될 가능성이 높다 — **측정 없이 구조를 먼저
설계하지 않는다**(§(e) 참고).

### (d) 참고 선례

`functions_bg_removal/main.py`의 `bg_removal_on_upload`가
`@storage_fn.on_object_finalized`(Storage 업로드 트리거, `asia-
northeast3`, `min_instances=0`)로 서버에서 자동으로 도는 구조다
— 새 옷 이미지가 업로드되는 시점에 자동으로 걸리는 방식의 직접
선례. 실시간 임베딩도 같은 트리거 지점(옷장 이미지 업로드)에
붙일 수 있어 보인다는 것이 §6 "(C) 실시간 임베딩 파이프라인"이
이미 등록해 둔 관측이다 — 다만 **콜드스타트·비용 프로파일이
같을지는 확인된 바 없다**(U2Net과 FashionCLIP은 모델 크기·연산
특성이 다르다).

### (e) 착수 전 측정할 것 후보 (측정 하네스 우선 원칙, 나열만)

- FashionCLIP 추론 시간 — CPU 전용 환경(로컬 또는 Cloud
  Functions/Run 유사 사양)에서 이미지 1장 처리 시간.
- 콜드스타트 지연 — `min_instances=0`으로 배포했을 때 첫 호출
  지연이 배경 제거 사례(참고할 실측 있으면 그 수치)와 비교해
  어느 정도인지.
- 상시 인스턴스(`min_instances>=1`) 비용 — Cloud Run/Functions
  과금 기준으로 월 예상 비용.
- 모델 파일 크기·메모리 요구량 — 배포 패키지에 실을 수 있는
  크기인지(배경 제거의 `u2netp.onnx`처럼 저장소에 포함할지,
  별도 스토리지에서 로드할지).
- 기존 배치 파이프라인(export→Kaggle→backfill) 결과와의 벡터
  일치성 — 실시간 경로가 만드는 벡터가 배치 경로가 만든 벡터와
  같은 분포/스케일인지(다르면 기존에 백필된 벡터와 신규 벡터가
  섞였을 때 비교가 깨질 수 있다).
